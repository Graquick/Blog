                  ...
                  {post.map((post, i) => (
                      <div key={i} className="mb-12">
                          <div className="flex flex-col gap-2">
                              <Link href={post.href}>
                                  <motion.h2 className="font-bold text-[1.5rem] cursor-pointer w-fit" whileHover={{ scale: 1.125, color: "#0ea5e9", x: -10, y: -10 }} whileTap={{ scale: .5 }}>{post.title} <Icon icon={post.icon} /></motion.h2>
                              </Link>
