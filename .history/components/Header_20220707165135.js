    ...
    <>
        <main className="left-0 m-auto right-0 py-8 w-full mb-8 h-[8rem] flex justify-between items-center">
            ...
            <nav className="h-fit">
                ...
                <ul className="hidden invisible flex-col gap-4 xl:flex xl:visible">
                    {linksMobile.map((linksM, i) => (
                        <div key={i}>
                            <li className={router.pathname === linksM.href ? "underline-offset-4 underline" : ""}>
                                <Link href={linksM.href}>
                                    <div>   
                                       {linksM.title}<Icon icon="dashicons:arrow-down-alt2" />
                                    </div>
                                </Link>
                            </li>
                        </div>
