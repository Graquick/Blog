            ...
            <nav className="h-fit">
                ...
                <ul className="hidden invisible flex-col gap-2 xl:flex xl:visible">
                    {linksMobile.map((linksM, i) => (
                        <div key={i}>
                            <li className={router.pathname === linksM.href ? "underline-offset-4 underline visible" : "hidden invisible"}>
                                <Link href={linksM.href}>
                                    <div className="flex gap-1 items-center cursor-pointer">   
                                       {linksM.title}{router.pathname === linksM.href && <motion.div variants={iconAnim} initial="closed" animate={open ? "opened" : "closed"}><Icon icon="dashicons:arrow-down-alt2" /></motion.div>}
                                    </div>
                                </Link>
                            </li>
                        </div>
